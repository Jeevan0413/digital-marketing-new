      ...
      <section className="bg-soft-1 reveal fade-in-up" style={{ padding: '120px 20px' }}>
        <div className="container">
          ...
          <div style={{ position: 'relative', marginTop: '60px', maxWidth: '860px', margin: '60px auto 0' }}>
            <div style={{ position: 'absolute', left: '50%', top: 0, bottom: 0, width: '2px', background: 'linear-gradient(to bottom, #6366F1, #0288D1, #26C6DA, #A855F7, #10B981)', transform: 'translateX(-50%)', borderRadius: '2px' }} />
            {journey.map((item, idx) => (
              <div key={idx} onClick={() => setActiveStage(idx)} style={{ display: 'flex', justifyContent: idx % 2 === 0 ? 'flex-start' : 'flex-end', marginBottom: '48px', position: 'relative', cursor: 'pointer' }}>
                <div style={{ position: 'absolute', left: '50%', top: '24px', width: '18px', height: '18px', borderRadius: '50%', background: item.color, border: '4px solid #fff', boxShadow: activeStage === idx ? `0 0 0 6px ${item.color}30` : `0 0 0 3px ${item.color}40`, transform: activeStage === idx ? 'translateX(-50%) scale(1.4)' : 'translateX(-50%)', zIndex: 2, transition: 'all 0.3s' }} />
                <div style={{ width: '44%', padding: '28px 32px', borderRadius: '20px', background: '#fff', boxShadow: activeStage === idx ? `0 20px 60px ${item.color}20` : '0 4px 24px rgba(0,0,0,0.06)', border: `2px solid ${activeStage === idx ? item.color : 'transparent'}`, transition: 'all 0.3s' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '12px' }}>
                    <span style={{ fontSize: '1.8rem', fontWeight: 900, color: item.color }}>{item.year}</span>
                    <h4 style={{ margin: 0, fontSize: '1.1rem', color: 'var(--text-main)' }}>{item.title}</h4>
                  </div>
                  <p style={{ color: 'var(--text-muted)', margin: 0, lineHeight: 1.7 }}>{item.desc}</p>
                </div>
              </div>
            ))}
          </div>
